...
static char version_notice[] = "Copyright (C) 2004-2007 Michal J. Kubski\n"\
...
class bandwidth_manager {
  ...
  private:
	...
	_call.tc(OBOC + "0 parent 1:2 classid 1:" + i + " htb  rate " +
	   ...
	   cfg.speed_unit + " burst " + to_string(cfg.up_burst) + " cburst " + to_string(cfg.up_cburst));
	...
	_call.tc(OBOC + "1 parent 1:2 classid 1:" + i + " htb rate " +
	  ...
	  cfg.speed_unit + " burst " + to_string(cfg.down_burst) +
	   " cburst " + to_string(cfg.down_cburst));

	_call.tc(OBOQ + "1 parent 1:" + i + " handle " + i + ": sfq perturb 30");
  ...
  public:
	...
	_call.ip(" link set dev imq0 txqueuelen 200 ");
	_call.ip(" link set dev imq1 txqueuelen 200 ");



	_call.tc(OBOQ + "0 root handle 1: htb r2q 4 default 6");
	...
	_call.tc(OBOC + "0 parent 1: classid 1:2 htb rate " + to_string(UP) +
	   ...
	   " burst " + to_string(cfg.up_burst) + " cburst " + to_string(cfg.up_cburst));
	...
	_call.tc(OBOQ + "0 parent 1:6 handle 6: sfq perturb 30 ");

	_call.tc(OBOQ + "0 parent 1:3 handle 3: sfq perturb 30 ");

	_call.tc(OBOQ + "0 parent 1:4 handle 4: sfq perturb 30 ");

	_call.tc(OBOQ + "0 parent 1:5 handle 5: sfq perturb 30 ");

	/* Download */
	_call.tc(OBOQ + "1 root handle 1: htb r2q 4 default 6");
	...
	_call.tc(OBOC + "1 parent 1: classid 1:2 htb  rate " +
	   ...
	   to_string(cfg.down_burst) + " cburst " + to_string(cfg.down_cburst));
	...
	_call.tc(OBOQ + "1 parent 1:6 handle 6: sfq perturb 30 ");

	_call.tc(OBOQ + "1 parent 1:3 handle 3: sfq perturb 30 ");

	_call.tc(OBOQ + "1 parent 1:4 handle 4: sfq perturb 30 ");

	_call.tc(OBOQ + "1 parent 1:5 handle 5: sfq perturb 30 ");
/*
	string buffer = "*mangle\n";		
	...
	_call.iptables_atomic(buffer);
*/
	_call.iptables(" -t mangle -N SHAPER_DOWN");
	_call.iptables(" -t mangle -N SHAPER_UP");
	_call.iptables(" -t mangle -I POSTROUTING -o " + cfg.lan + " -j SHAPER_DOWN");
	_call.iptables(" -t mangle -I PREROUTING -i " + cfg.lan + " -j SHAPER_UP");	
	_call.iptables(" -t mangle -A SHAPER_DOWN -o " + cfg.lan + " -j IMQ --todev 1");
	_call.iptables(" -t mangle -A SHAPER_UP -i " + cfg.lan + " -j IMQ --todev 0");
